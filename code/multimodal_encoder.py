...
class MMEncoder(nn.Module):
    ...
    def __init__(self, in_size, out_size, enc_psize=[], enc_hsize=[], att_size=100,
        ...
        for m in six.moves.range(len(in_size)):
            ...
            self.lstm_list.append(nn.LSTM(enc_hsize_, enc_hsize_, batch_first = True))
    ...
    def simple_modality_fusion(self, c, s):
        ...
        c_frame = self.lgd[0](F.dropout(c[0]))
        c_flow = self.lgd[1](F.dropout(c[1]))
        c_audio = self.lgd[2](F.dropout(c[2]))
        return c_frame, c_flow, c_audio
    ...
    def __call__(self, s, x, train=True):
        ...
        v_frame, v_flow, v_audio = self.simple_modality_fusion(c, s)
        return torch.tanh(v_frame), torch.tanh(v_flow), torch.tanh(v_audio)
